...
```shell
deployments/local/start-gemfire-local.sh
```
Create region
...
```shell
$GEMFIRE_HOME/bin/gfsh -e "connect" -e "deploy --jar=$PWD/components/server/gemfire/trader-functions/build/libs/trader-functions-0.0.1-SNAPSHOT-all.jar"
```
